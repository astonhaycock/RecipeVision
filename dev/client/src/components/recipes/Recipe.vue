...
async function getGenerateRecipes() {
  ...
  const response = await fetch(
    `${import.meta.env.VITE_PUBLIC_URL}/api/ai/recipe`
  );
...
async function generateRecipes() {
  const response = await fetch(
    `${import.meta.env.VITE_PUBLIC_URL}/api/recipe/generate`
  );
  if (response.status === 200) {
    const data = await response.json();
    ai_recipes.push(data);
        ...
        ><div id="recipe_modal" v-if="modal">
          <div id="paper" class="d-flex flex-column ga-2 overflow-auto">
            ...
            <div
              ...
              :class="
                mobile ? 'flex-column justify-center align-center' : 'flex-row'
              "
            >
              <v-img
            ...
            <div>
              ...
              >
                <div
                  ...
                  :class="mobile ? 'w-90' : 'w-50'"
                >
                  <h2>Instructions</h2>
                  <ol class="ml-8">
                    <li
                      v-for="instruction in modal.instructions"
                      :key="instruction"
                    >
                      {{ instruction }}
                    </li>
                ...
                <div class="d-flex flex-column">
                  ...
                  <ul class="ml-8">
                    <li
                      v-for="ingredient in modal.ingredients"
                      :key="ingredient"
                    >
                      {{ ingredient }}
                    </li>
